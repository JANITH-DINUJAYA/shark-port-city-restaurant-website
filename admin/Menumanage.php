...
<head>
    ...
    <style>
        ...
        .main-content { flex-grow: 1; padding: 50px; display: flex; flex-direction: column; align-items: center; }
        .header-bar { width: 100%; max-width: 1000px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; }
        .back-button { background-color: var(--primary-dark); color: var(--white); border: none; padding: 10px 20px; border-radius: 8px; text-decoration: none; transition: background-color 0.3s ease; }
        .back-button:hover { background-color: #34495e; }
        .menu-form-container, .menu-list-container { background-color: var(--white); border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); padding: 40px; width: 100%; max-width: 1000px; text-align: left; margin-bottom: 30px; }
        .menu-form-container h1, .menu-list-container h1 { font-size: 2rem; font-weight: 600; color: var(--primary-dark); margin-bottom: 25px; }
        .form-group { margin-bottom: 20px; }
        .form-group label { display: block; font-weight: 600; margin-bottom: 8px; color: var(--text-color); }
        .form-group input, .form-group textarea, .form-group select { width: 100%; padding: 12px; border: 1px solid #ccc; border-radius: 8px; font-size: 1rem; transition: border-color 0.3s ease; }
        .form-group input:focus, .form-group textarea:focus, .form-group select:focus { outline: none; border-color: var(--accent-green); box-shadow: 0 0 0 3px rgba(46,204,113,0.2); }
        .form-group textarea { resize: vertical; min-height: 120px; }
        .submit-button, .action-button { background-color: #f70404; color: var(--white); border: none; padding: 15px 30px; font-size: 1.1rem; border-radius: 8px; cursor: pointer; display: block; width: 100%; font-weight: 600; text-align: center; }
        .submit-button:hover { background-color: #f70404; }
        .delete-button { background-color: var(--accent-red); }
        .delete-button:hover { background-color: #c0392b; }
        .update-button { background-color: #000000ff; }
        .update-button:hover { background-color: #000000ff; }
        .menu-table { width: 100%; border-collapse: collapse; margin-top: 20px; }
        .menu-table th, .menu-table td { padding: 12px 15px; border: 1px solid #ddd; text-align: left; }
        .menu-table th { background-color: #000000ff; color: var(--white); font-weight: 600; }
        .menu-table tr:nth-child(even) { background-color: #f9f9f9; }
        .menu-table tr:hover { background-color: #f1f1f1; }
        .menu-table img { max-width: 80px; height: auto; border-radius: 4px; }
        .menu-actions { display: flex; gap: 10px; }
        .menu-actions a { padding: 8px 12px; font-size: 0.9rem; text-decoration: none; color: white; border-radius: 5px; display: inline-block; text-align: center; }
    </style>
</head>
<body>
    <nav>
                    ...
                    <?php foreach($menuItems as $id => $data): ?>
                        <tr>
                            ...
                            <td class="menu-actions">
                                ...
                                <a href="delete_item.php?id=<?= $id ?>" class="action-button delete-button" onclick="return confirm('Are you sure you want to delete this item?');">Delete</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
